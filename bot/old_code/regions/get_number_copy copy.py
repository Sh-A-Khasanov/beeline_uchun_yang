...
def main():
    ...
    try:
        for wh in warehouses:
            ...
            page = 1
            size = 100
            phone_number_mask = "9989*******1"
            while True:
                # Parametrlarni shu yerda yig'amiz
                params = {
                    "page": page,
                    "size": size,
                    "warehouse_code": warehouse_code,
                    "phone_number_mask": phone_number_mask  # <-- MANA SIZGA KERAKLI PARAMETR
                }            
                
                # full_url = f"{API_BASE}?page={page}&size={size}&warehouse_code={warehouse_code}&phone_number_mask={phone_number_mask}"
                ...
                logger.info(f"So'rov yuborilmoqda: {full_url}")
                ...
                if status_code == 429 or status_code is None:
                    logger.warning("429 yoki so'rov muammosi. User-Agent yangilanmoqda.")
                    ua_idx += 1
                    if ua_idx >= len(ua_list):
                        logger.warning("Barcha UA lar tugadi. Proxy yangilanmoqda.")
                        ua_idx = 0
                        proxy_idx += 1
                        if proxy_idx >= len(proxy_list):
                            logger.warning("Barcha proxylar tugadi. Yangi proxylar yuklanmoqda.")
                            new_proxies = fetch_and_check_proxies(max_good=40, workers=10)  # Workers reduced
                            if not new_proxies:
                                logger.error("Yangi proxy topilmadi. To'xtatilmoqda.")
                                return
                            proxy_list = new_proxies
                            proxy_idx = 0
                            logger.info(f"Yangi proxylar topildi: {len(proxy_list)} ta.")
                    ...
                    try:
                        driver.quit()
                        logger.info("Old driver successfully quit.")
                    except Exception as e:
                        logger.warning(f"Driver quit failed: {e}. Ignoring.")

                    try:
                        driver = start_selenium(proxy=current_proxy, user_agent=current_ua)
                        logger.info("New driver started.")
                    except Exception as e:
                        logger.error(f"New driver start failed: {e}. Skipping retry.")
                        continue  # Or break if you want to stop

                    cookie_header = cookies_dict_to_header(get_cookies_from_driver(driver))

                    time.sleep(10 + random.randint(0, 10))  # Increased sleep
                    continue

                if status_code != 200:
                    logger.warning("200 qaytmayapti. Keyingi warehouse ga o'tish.")
                    time.sleep(2)
                    break

                try:
                    data = resp.json()
                    logger.debug("JSON muvaffaqiyatli parse qilindi.")
                except Exception as e:
                ...
                if items:
                    ...
                    logger.info(f"{len(items)} ta raqam saqlandi ({page} / {total_pages} page)")
                else:
                    logger.info("Sahifada item topilmadi.")

                if page >= total_pages:
                    logger.info(f"Warehouse uchun barcha sahifalar tugadi (total_pages={total_pages}).")
                    break
                page += 1
